...
def grab_still(render_path:str,temp_path:str):
    ...
    timeline_framerate=int(current_timeline.get_setting('timelinePlaybackFrameRate'))
    ...
    for still in still_dict:
        ...
        if _file_prefix in file_name_list:
            ...
            current_album.export_stills(gallery_stills= [still],folder_path= TEMP_PATH,file_prefix=_file_prefix,format=StillFormats.TIF)
...
if __name__ == "__main__":
    paser=argparse.ArgumentParser('grab still from davinci resolve timeline marker')
    paser.add_argument('-renderPath','-rP',required=True,help='prores 4444 mov still render path')
